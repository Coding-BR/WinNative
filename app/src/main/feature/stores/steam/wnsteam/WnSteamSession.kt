...
class WnSteamSession : AutoCloseable {
    ...
    // Send CMsgClientLogOff + flush before disconnect. Bare disconnect leaves
    // the session "alive" on Steam's side for ~5 min, which makes the next
    // LogonWithRefreshToken hit EResult=15.
    fun logOffAndDisconnect(flushMs: Int = 500) {
        val h = nativeHandle.get(); if (h == 0L) return
        nativeLogOffAndDisconnect(h, flushMs)
    }

    // Mint a FRESH refresh token via Authentication.GenerateAccessTokenForApp#1
    // so a follow-up logon bypasses Steam's concurrent-login anti-abuse.
    // Blocking — MUST run on Dispatchers.IO. The OLD token is invalidated on
    // success; the caller MUST persist the new one before disconnect.
    ...
    // Returns [totalBytes, usedBytes] or null. BLOCKING — off main thread.
    ...
    // Rename — server echoes the new name back via getSelfPersona().
    ...
    // personaStateRequested is an EClientPersonaStateFlag bitmask (1=PlayerName).
    // Replies arrive async; read them back via [getFriendPersonas].
    ...
    // Mutual friends only (EFriendRelationship=3); empty until post-logon push.
    ...
    // Cached friend personas as `[{sid,name,state,app},...]`; "[]" until push.
