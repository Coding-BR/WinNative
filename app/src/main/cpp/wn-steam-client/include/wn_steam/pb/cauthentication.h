...
struct CAuthentication_AccessToken_GenerateForApp_Request {
    std::string           refresh_token;
    uint64_t              steamid       = 0;
    EAuthTokenRenewalType renewal_type  = EAuthTokenRenewalType::None;

    [[nodiscard]] std::vector<uint8_t> serialize() const;
};

struct CAuthentication_AccessToken_GenerateForApp_Response {
    std::string access_token;
    std::string refresh_token;          // empty unless renewal_type=Allow

    [[nodiscard]] static std::optional<CAuthentication_AccessToken_GenerateForApp_Response>
    deserialize(std::span<const uint8_t> body) noexcept;
};
